...
class Resource:
    ...
    def __call__(  # pylint: disable=too-many-locals  # noqa: C901
        ...
        total_ms = context.get_remaining_time_in_millis()
        logs_setup = False
        ...
        try:
            ...
            logs_setup = True
            elapsed = int((total_ms - context.get_remaining_time_in_millis())/1000)
            LOG.error(f"-------------------- PLUGIN START: logs_setup "
                      f"{total_ms} {elapsed} "
                      f"{event.action} "
            ...
            try:
                elapsed = int((total_ms - context.get_remaining_time_in_millis()) / 1000)
                LOG.error(f"-------------------- PLUGIN START: invoking_handler "
                          f"{total_ms} {elapsed} "
                          f"{event.action} "
                          f"{context.aws_request_id}"
                          f"-------------------------")
                h_start = context.get_remaining_time_in_millis()
                progress = self._invoke_handler(caller_sess, request, action, callback)
                h_end = context.get_remaining_time_in_millis()
                elapsed = int((h_start - h_end) / 1000)
                LOG.error(f"-------------------- HANDLER TIME: {elapsed} "
                          f"{event.action} "
                          f"{context.aws_request_id}"
                          f"-------------------------")
                elapsed = int((total_ms - context.get_remaining_time_in_millis()) / 1000)
                LOG.error(f"-------------------- PLUGIN START: handler_done "
                          f"{total_ms} {elapsed} "
                          f"{event.action} "
                          f"{context.aws_request_id}"
                          f"-------------------------")
            ...
            progress = ProgressEvent.failed(HandlerErrorCode.InternalFailure, traceback.format_exc())
        except BaseException as e:  # pylint: disable=broad-except
